...
#### 변경 내용
- **`dvbuild stop` 추가 — dvbuild로 건 빌드만 중단한다.** 사용자가 터미널에서 직접 건 빌드는 건드리지 않는다.

#### 사용법
- `dvbuild stop` — dvbuild가 건 빌드 전부 중단 / `dvbuild stop 754` — 그 프로젝트 것만
- SIGINT(Ctrl+C와 동일)를 먼저 보내고, 5초 뒤에도 남아 있으면 SIGKILL.

#### 구현 메모
- 실행할 때 `docker exec -e DVBUILD_ID=<고유값>`으로 **표식을 심는다.** 자식 프로세스가 환경변수를 물려받으므로 `/proc/*/environ`을 훑어 그 표식이 있는 것만 골라낸다.
- **이름으로 `pkill -f 'dvbuild_mode.sh'` 하면 안 된다.** 2026-08-28에 실제로 사용자가 직접 건 빌드까지 같이 죽였다. 이름은 같아도 주인이 다르다.
- `docker top`은 호스트 PID를, `/proc` 스캔은 컨테이너 PID를 준다. `kill`은 컨테이너 안에서 걸어야 하므로 컨테이너 PID를 쓴다.
...
#### 변경 내용
- 실행 방식을 고를 수 있다. 기본은 **새 터미널 창**, `-b`는 창 없이 백그라운드, `-n`은 현재 셸에서 붙잡고 실행.
- 로그 링크를 **프로젝트별로** 만든다. 609H와 754를 동시에 돌려도 `dvbuild log 754`로 골라 본다.

#### 사용법
- `dvbuild 754` — 새 창에서 (기본)
- `dvbuild -b 609h dvmgmt` — 창 없이 백그라운드
- `dvbuild log` / `dvbuild log 754` — 최근 로그 / 그 프로젝트 최근 로그

#### Fixed / Changed
- **기존 터미널 창에 탭으로 붙이는 건 불가능하다.** gnome-terminal 3.x는 서버 구조라 `--tab`이 한 명령줄 안에서만 묶인다. 밖에서 호출하면 두 번 시험해도 전부 새 창이 떴다. xdotool로 창을 활성화하고 `Ctrl+Shift+T`를 넣는 방법은 키보드·한글 IME를 뺏으므로 쓰지 않는다.
- `latest` 심볼릭 링크가 프로젝트 공용이라 동시 빌드 때 엉뚱한 로그를 가리키던 문제 → `latest_<proj>` 추가.
...
## scripts (scripts/migrate_test_to_hdd.sh)

### [1.0.0] — 2026-08-28

#### 변경 내용
- 신규. `~/test`(3.5G)를 `/hdd/ksc/test` 로 병합하고 심볼릭 링크로 잇는다. `/` 가 92% 찼는데 `/hdd` 는 322G 여유다. **경로가 안 바뀌므로 문서·메모리·스킬을 하나도 안 고쳐도 된다.**
- **지금은 실행 못 한다** — `~/test/S1KTHOME-1440/무인시험_20260806/` 수집 스크립트가 3일째 돌고 있다. 그래서 "나중에 안전할 때 실행" 이 가능하도록 안전조건을 스크립트 안에 박았다.

#### 안전조건 (`run` 이 스스로 거부한다)
- `~/test` 안의 파일을 연 프로세스가 있으면 거부. **다른 디스크로 옮기는 건 복사 후 원본 삭제라, 실행 중인 스크립트가 열어둔 파일은 삭제된 원본에 계속 쓰게 되어 수집분이 사라진다.**
- 최근 10분 안에 수정된 파일이 있으면 거부 (다른 세션 작업 중)
- `/hdd` 여유 부족이면 거부

#### 아무것도 잃지 않게 하는 장치
- `/hdd/ksc/test` 에 이미 15G(2100 시험)가 있어 **단순 이동이 아니라 병합**이다. 덮어쓰는 파일은 `--backup-dir=.merge_backup_<날짜>/` 로 먼저 보관한다.
- `run` 후에도 원본은 `~/test.pre_hdd_<날짜>` 로 남긴다. 며칠 써보고 `finish` 를 해야 지운다.
- 검증(`rsync --dry-run` 잔여 0건)을 통과하지 못하면 원본을 건드리지 않는다.
...
- **`~/.devtools/` 추가** — 도구 설정(noti 웹훅·봇토큰, ftd AP 접속정보, aptest, cpbak, scs, spec). 이게 없으면 디스크가 죽었을 때 웹훅·봇토큰 재발급부터 다시 해야 한다. **사용자 결정(2026-08-28): 비공개 저장소이므로 비밀값까지 통째로 백업한다.** 로그·산출물(`*.log`, `runs/`, `artifacts/`)은 매일 바뀌기만 하고 복구 가치가 없어 제외 — 넣으면 100K 짜리가 1.4M 로 불어나고 매일 커밋이 지저분해진다.
...
#### 변경 내용
- **전달자 모드.** 알림에 `-n <세션이름>` 을 붙여 보내면, 사용자가 그 알림에 답장했을 때 봇이 답하지 않고 **그 세션에 그대로 넘긴다.** 그 세션은 자기 맥락을 갖고 있어 이어서 작업할 수 있다. 실측 12초(12:56:19 수신 → 12:56:31 전달 완료).
- 그래서 **무인 작업마다 Monitor 감시를 거는 절차가 필수가 아니게 됐다.** 세션이 살아있고 `-n` 만 붙였으면 폰에서 바로 꽂힌다. 세션이 죽었으면 지금까지처럼 봇이 읽기 전용으로 답한다.
- `NOTI_BOT_FORWARD=off` 로 끄면 항상 봇이 답한다.

#### 왜 이렇게 만들었나
- **세션 UUID 로는 다른 세션을 부를 수 없다.** 부를 때 쓰는 건 `memo-6b` 같은 이름인데, 이름은 `폴더명-무작위2자` 라 UUID 에서 계산이 안 된다. 그래서 `noti -n` 으로 보낼 때 이름을 같이 적어 `threads.tsv` 6열에 남긴다.
- **봇은 셸이라 세션을 못 부른다.** `SendMessage` 는 Claude 도구다. 그래서 `claude -p --allowedTools "ListAgents,SendMessage"` 로 전달자를 한 번 띄운다. 헤드리스 세션에서 `SendMessage` 가 되는 것은 실측 확인했다.
- 21시간 idle 이던 세션도 메시지로 깨어나 응답하는 것을 확인했다.
